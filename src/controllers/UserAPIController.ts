  ...
  async getGenerationHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!service || service === 'all') {
        ...
        tasks = [
          ...freepik.map(t => ({ ...t, service: 'freepik' })),
          ...midjourney.map(t => ({ ...t, service: 'midjourney' })),
          ...runway.map(t => ({ ...t, service: 'runway' }))
        ].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
         .slice(0, limit);

      } else {
        // Получаем задачи конкретного сервиса
        switch (service) {
          case 'freepik':
            tasks = await prisma.freepikTask.findMany({
              where: { userId },
              orderBy: { createdAt: 'desc' },
              take: limit
            });
            break;
          case 'midjourney':
            tasks = await prisma.midjourneyTask.findMany({
              where: { userId },
              orderBy: { createdAt: 'desc' },
              take: limit
            });
          ...
          case 'runway':
          case 'kling':
            tasks = await prisma.runwayTask.findMany({
              where: { userId },
              orderBy: { createdAt: 'desc' },
              take: limit
            });
